...
describe('buildCaptureRow (the engine plumbing — math owned by cross-venue-arb.test.ts)', () => {
  it('assembles a complete, well-formed row from a matched (poly, kalshi) pair', () => {
    const { ladder: kalshi } = parseKalshiLadder(NYC_KALSHI_MARKETS, 'nyc', '2026-06-25');
    const row = buildCaptureRow('nyc', '2026-06-25', polyLadderFromEvent(NYC_POLY()), kalshi, NOW.toISOString());
