-- 0099_buy_table_cycles_rpc.sql — HOTFIX: move liveCycles out of dash_trading() into its own fail-soft RPC.
-- ══════════════════════════════════════════════════════════════════════════════════════════════════════════
-- INCIDENT (operator-reported 2026-07-12 ~00:20Z, minutes after 0098 went live): /trading rendered
-- "rpc dash_trading failed: canceling statement due to statement timeout" — the WHOLE console down.
-- ROOT CAUSE: 0098 inlined the liveCycles scan into dash_trading(). The page calls the RPC as the
-- `authenticated` role, whose statement_timeout is 8s (rolconfig, verified); the scan's real prod scope at
-- that hour was 91 live events / 16,558 opening_captures rows, each expanding a ~15-bucket jsonb with a
-- per-row argmax sort — too heavy on the saturated Micro inside the 8s budget. The 0098 pre-apply
-- verification ran via execute_sql as `postgres` (NO statement_timeout) and was therefore green — a false
-- pass. Lesson (the C16 data_freshness twin): verify RPC latency under the CALLING role's timeout, not the
-- superuser's.
--
-- WHAT:
--   1. dash_trading() — RESTORED VERBATIM to the 0097 body (no liveCycles key). The console must never
--      depend on the cycles scan: applying this migration alone brings /trading back, even before the web
--      redeploy (the deployed panel reads an absent liveCycles as its staged-dark note).
--   2. buy_table_live_cycles() — the SAME cycle aggregation as 0098, now:
--        · its own operator-guarded RPC returning the OBJECT envelope { cycles: [...] } (0081 tripwire),
--          called SEPARATELY and fail-soft by the /trading loader — a slow or absent cycles read degrades
--          ONLY the lo/hi columns, never the console;
--        · restricted to the PANEL-DISPLAYABLE city set — trade_config.city_allowlist ∪ the
--          buy_table.city_price_ranges override keys (exactly the 0097 rowSlugs the table can render; an
--          allowlist NULL means the panel lists overrides only — same here). On today's prod config that is
--          4 cities instead of 45: ~10× less scan for identical rendered output.
--      Per-tick semantics unchanged (the exact selectBuyTableCandidates pick: argmax houseProb among
--      identity-complete buckets, execAsk falling back to bestAsk, 0 < ask ≤ 1; unseeded/no-ask ticks
--      contribute nothing; jsonb_typeof guards — a mangled row scores null, never raises). Live = the
--      market_events row unresolved AND still trading (resolves_at > now()), target_date ≥ current_date − 2.
--
-- Grants: buy_table_live_cycles like dash_trading (service_role + authenticated; self-guards via
-- operator_guard; revoked from public/anon). dash_trading grants re-asserted identical to 0097.
-- No table, no cron, no edge-fn change (cron count stays 35). Idempotent-safe (create-or-replace only).
--
-- Rollback: drop function public.buy_table_live_cycles(); (dash_trading is already the 0097 body).
-- ══════════════════════════════════════════════════════════════════════════════════════════════════════════

-- ══════════════════════════════════════════════════════════════════════════════════════════════════════════
-- SECTION 1 · dash_trading() — the 0097 body VERBATIM (liveCycles removed; console decoupled from the scan)
-- ══════════════════════════════════════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════════════════════════════════════
-- SECTION 2 · buy_table_live_cycles() — the cycles scan as its own fail-soft RPC, panel-city-restricted
-- ══════════════════════════════════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- the override map, read FAIL-SOFT (the 0097 idiom — a hand-mangled config row never breaks the read).
  begin
    v_over := coalesce((select value from public.config where key = 'buy_table.city_price_ranges'), '{}')::jsonb;
    if v_over is null or jsonb_typeof(v_over) <> 'object' then
      v_over := '{}'::jsonb;
    end if;
  ...
  -- the PANEL-DISPLAYABLE city set: trade_config.city_allowlist ∪ the override keys (the 0097 rowSlugs).
  -- An allowlist NULL means the price-range table lists override cities only — mirrored exactly, so the
  -- scan never does work the panel cannot render.
  select coalesce(array_agg(distinct s), '{}'::text[]) into v_cities
  from (
    select lower(trim(u)) as s
    from unnest(coalesce((select t.city_allowlist from public.trade_config t where t.id = 1), '{}'::text[])) u
    union all
    select lower(trim(k)) from jsonb_object_keys(v_over) k
  ) src
  where coalesce(s, '') <> '';

  -- per (city, live target-date cycle): min/max of the LANE'S GATE PRICE over the cycle's ENTIRE live
  -- period — per opening_captures tick, the predicted bucket = argmax houseProb among identity-complete
  -- buckets (the exact selectBuyTableCandidates pick), its execAsk falling back to bestAsk, kept only when
  -- 0 < ask ≤ 1. Unseeded ticks contribute nothing — the lane could not have bought on them either.
  ...
  from (
    ...
    join public.cities c on c.id = e.city_id and c.slug = any (v_cities)
    cross join lateral (
