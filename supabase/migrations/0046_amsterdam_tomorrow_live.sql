-- 0046_amsterdam_tomorrow_live.sql — surface TOMORROW's prediction + the live "as-of-now" running max.
--
-- Operator review (2026-06-21): the /amsterdam page answered "best time to bet / who's winning / history"
-- but NOT the two most glanceable operational facts — what we predict for TOMORROW, and the running-max
-- floor as of right now (not just frozen at each past lock hour). Both inputs already flow:
--   · tomorrow's high  = bias-corrected cross-model lead-1 forecast from forecast_snapshots (the SAME blend
--     + trailing-30 debias the bet placement uses — amsterdam_sim_place_inputs, migration 0041), wuRound'd
--     to the whole-°C bucket and priced against tomorrow's live ladder.
--   · live running max = intraday_max (icao='EHAM', today) — max_tenths_c + last_obs_at ("as of HH:mm").
-- The dashboard read is RPC-only (the web port is RLS-scoped, never the service role — apps/web/src/lib/
-- api/deps.ts), so these must come from the RPC, not a second loader select. This migration redefines the
-- WHOLE 0043 dash_amsterdam_sim() body verbatim (create-or-replace; the 0043 truth panel + 0042 CIs are
-- preserved exactly) and ADDS two top-level blocks: `tomorrow` and `liveRunMax`. Everything else — config,
-- coverage, arms (+truth), leader, equity, betsByArm, truthByArm, betLog, latest, truthCoverage — is the
-- 0043 body unchanged. Signature unchanged (operator-authenticated read). No table changes.
--
-- The forecast math is a verbatim mirror of amsterdam_sim_place_inputs (0041): v_t_raw = cross-model mean
-- of lead-1 captures for the target; v_t_bias = mean residual over the trailing 30 finalized pairs strictly
-- before the target (>= 20 pairs required, else the correction is too noisy). UNLIKE placement (which nulls
-- the forecast below 20 pairs and falls back to the floor), the DISPLAY falls back to the RAW forecast so a
-- tomorrow number always shows when the NWP feed has the day — with biasCorrected=false flagging it.
...
declare
  v jsonb;
  -- 0046: tomorrow forecast + live running-max scratch
  ...
  v_t_nmod   int;       -- how many model captures the mean is over
...
begin
  ...
  -- ── 0046: tomorrow's prediction (bias-corrected lead-1 forecast → bucket → live odds) ────────────────
  -- Cross-model lead-1 mean for tomorrow (uncorrected) + how many captures behind it.
  select avg(fs.tmax_c), count(*)
    into v_t_raw, v_t_nmod
  from public.forecast_snapshots fs
  where fs.icao = 'EHAM' and fs.target_date = v_tmrw and fs.lead_days = 1;

  -- Trailing-30 lead-1 bias = mean(actual − forecast) over finalized days strictly before tomorrow
  -- (verbatim mirror of amsterdam_sim_place_inputs / migration 0041; >= 20 pairs to trust).
  with fc1 as (
  ...
  -- Display fallback: corrected when trustworthy, else the raw forecast (so a number always shows).
  ...
  -- Tomorrow's Amsterdam "highest" market + the ladder bucket the call lands in + its latest ask.
  ...
  -- ── 0046: live running max as of now (intraday_max — the real-time floor + its observation stamp) ─────
  select jsonb_build_object(
    'date',       im.date_local,
