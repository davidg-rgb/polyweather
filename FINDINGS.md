...
| **OBS-TRANSMISSION — the 5-min obs feed LEADS the real book (~5–25 min), but the taker form is NOT an edge; the market's residual bid on "obs-dead" buckets is correctly-priced resolution-source insurance** (2026-07-25, two passes on the Synoptic trial corpus, 11 US cities / 07-19..25. Pass 1 (trade prints, 3.09M pts): floor-kill collapse concentrates AFTER the 5-min print (−0.5¢ pre vs −6.0¢ post-15m, 88% post). Pass 2 (real `opening_captures` bids, 66 city-days / 354 kills): timing CONFIRMED on quotes (median pre-print drift 0.0000) — but "buy NO on fresh kills" at walked execBid has clustered CI straddling 0 at margins 1–2 ([−0.095,+0.266] / [−0.149,+0.359]) because 19/19 winner-"kills" (°C→°F/WU-source fabrications, 5.4% of kills; SF/Austin/Chicago cluster) kept 0.05–0.93 bids and the MARKET WON every one — adverse selection by someone who understands the resolution source. Margin ≥3°F zeroes fabrications in-window and reads +0.215/$1 CI [+0.111,+0.385] — but it is a CONSTANT-OUTCOME cell (all 14 trades won; CI = price dispersion, not risk; the 07-24 tool-guard trap) at a $52–156/WEEK pot: **INSUFFICIENT, no build, no §13 reopen**. Denominator the mid-basis pass hid: only ~22% of obs-kills had ≥5¢ real bids left at T−30. Re-adjudicate ~08-06 on the grown corpus (3× window) before the trial dies ~08-08. **↳ Same evening: the fabrication mechanism RESOLVED by the operator's resolution-oracle doc + in-house validation (66/66 winner replication off IEM's METAR/SPECI stream; synoptic 5-min max EXCEEDS the METAR-table max on 42% of days)** — WU's table renders METAR/SPECI ONLY, so the winner-"kills" were 5-min blips resolution never sees, and the market's residual bids were sharps who know the oracle. Shipped: synoptic lane → CAPTURE-ONLY (wrong-grade `intraday_max` writes removed, contaminated rows re-floored METAR-grade); the 08-06 re-run gets a fabrication-free METAR-grade kill rule with the 5-min stream as anticipatory trigger (`OBS-TRANSMISSION.md` addendum + `docs/DATA-SOURCES.md` §resolution-oracle) | `OBS-TRANSMISSION.md` (+ `synoptic-realbook-crosscheck.py`, `oracle-replica-validation.py`, result txt local) | — |
