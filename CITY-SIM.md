...
|---|---|---|---|---|---|
| **Singapore** | WSSS | 0.642 | **+0.027** (we edge it) | ~12:30 | 11 / 12 / 13 / 14 |
| **Karachi**   | OPKC | **0.619** (2nd-most accurate) | −0.045 (market sharper) | ~12:24 | 11 / 12 / 13 / 14 |
...
floor is locked by ~13:00 local, so unlike Amsterdam (peak ~13:45, arms 13–16) they race **11–14** — bracketing
the real peak instead of betting after it (where the market has already priced the certain outcome, ~no payout).
...
1. Apply `0070_city_paper_sim.sql` to prod (`apply_migration` MCP, or `npx supabase db push`).
...
3. Seed the curve: `pnpm tsx scripts/city-sim.ts` (then the 10:00-UTC cron carries it forward).
