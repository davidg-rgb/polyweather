...
export function replayGoogleBracket(
  ...
  return {
    ...
    // the entry-tick book (this engine's entry is a same-tick taker, so the fill tick IS the decision tick).
    entryBestBid: fin(entryBucket.bestBid) ? entryBucket.bestBid : NaN,
    entryExecBid: fin(entryBucket.execBid) ? entryBucket.execBid : NaN,
    entryDepthUsd: fin(entryBucket.depthUsd) ? entryBucket.depthUsd : NaN,
    entrySellbackDepthUsd: fin(entryBucket.sellbackDepthUsd) ? entryBucket.sellbackDepthUsd : NaN,
    entryLabel: entryBucket.label,
    bucketIdx: bIdx,
    wouldHaveWonAtResolution:
      input.resolution.gradingMismatch || input.resolution.winnerIdx == null ? null : input.resolution.winnerIdx === bIdx,
    exitReason,
    exitPrice,
    netPnlUsd,
    stakeUsd,
