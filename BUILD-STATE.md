...
> **☀️ 2026-07-03 ~05:40Z MORNING SUMMARY — the overnight loop's verdicts + the incident + WHAT REMAINS.**
> **Done (5 commits pushed green, suite 1818):** ① **Phase-0.5 spike = NO-GO** (0/325, Wilson [0%,1%]) — the flat-open entry is formally dead; FINDINGS.md updated; the maker-exit forward loop is the 12th signal's sole surviving measurement. ② **Jackknife**: the backtest PASS is real but marginal (mean +6.1% survives every exclusion; 24/65 single exclusions tip ciLow<0); **day-block tightening built (OPT-IN `dayBlockNull`) and PASSES** (day CI [+2.4%,+12.6%]). ③ **Ledger decomposition**: the edge IS the maker-TP leg (+$1,543 vs −$1,028 drag); live maker-fill rate (49% backtest vs 0.30 early live) decides everything; median win-sell rest ~16h. ④ Incident fixed at 3 layers (0076 applied · v4 redeployed · panels paused + 7 wedged rows cleaned — all operator-authorized ~05:20Z).
> **REMAINING (in order, operator-approved):**
> 1. **Recovery watch**: capture ticks complete but still `inserted:0` at 05:24Z (254s/tick, seed RPCs crawling) — the disk-IO budget refills over HOURS. When capture inserts are healthy ~30 min → **re-enable both panel crons**: `select cron.alter_job(jobid, active := true) from cron.job where jobname in ('maker-exit-panel','convergence-panel');` (they were paused via alter_job ~05:19Z; v4+0076 make the ticks cheap).
> 2. **21-day baseline**: archive pull for 07-02 is DONE (48 events, 152k pts, on disk); once the DB breathes → `pnpm tsx scripts/research/sim-maker-exit.ts --build-cache` then `--from-cache --split --tp 0.12 --sl 0.20 --tstop-hours 18 --chw 0 --max-entry 0.30 --depth 150 --maker-window 30 --rebate 0` + re-run `jackknife-maker-exit.ts` — does the PASS hold at 21 days? Record in MAKER-EXIT-SIM.md banner.
> 3. **NEW operator-requested sim (after #2): the FLUCTUATION-TAKER variant** — pair the archive with forecasts at leads 2/1/0, re-center the key bucket set (predicted Tmax ±1°C) per lead, taker entries/exits from PRICE-PATH signals (dip-depth × momentum-window × exit-rule sweep, incl. path-based exits), calibrated synthetic book + real fee curve, frozen §9R-E gate + OOS date split + day-block tightening. First pass on the 20-min cache; rebuild finer (per-minute) only for survivors. Context: the corrected archive moved taker breakeven ×0.70→×1.14 (mean-positive at real spread, CI-blocked) so path variants are NOT pre-doomed; but dip-buys rhyme with §12 adverse selection and delayed entry was monotone toxic — expect a fight.
> 4. Morning /remember entry — DONE this session; write a fresh one when #1–#3 close.
...
> - **Spike infrastructure fix (in the same pass):** `opening-spike.ts` `loadSeries` no longer calls the 0068 `bot_spike_series` RPC — its single-shot CTE materializes the ~3.4 KB TOASTed `buckets` BEFORE the row_number filter (~1.2 GB window sort at 45-city scale), which ran for minutes, saturated the pooler (two zombied runs: server-side query died, client hung on the dead socket; collateral MCP connection timeouts). Replaced with a two-stage direct-SQL read: slim-column window (no detoast) → fat rows by PK in 2,000-row chunks (each sub-second, per-chunk retry). Whole spike now completes in ~2 min.
> - **🔴 MORNING ACTION #2 — apply migration `0076_capture_read_scaling.sql`** (`eab6895`): the SAME TOAST-through-the-window-sort trap fixed at the SQL root for BOTH readers — `bot_spike_series` (the 1.2 GB sort) and `convergence_capture_inputs` (the ~3–8 s/city that × 45 sequential cities pushed the panel tick past the isolate wall). Slim-window rank → PK join-back; signatures/contracts/grants byte-identical (0068+0073 twins pass unchanged). Apply via MCP `apply_migration`, THEN redeploy `maker-exit-panel` (action #1) — together the tick should drop from 400s+ (dead) to well under a minute.
> - **✅ 05:20Z MORNING RESPONSE (operator-authorized, operator awake):** (1) migration **0076 APPLIED** to prod; (2) **maker-exit-panel v4 REDEPLOYED** (bounded fetch); (3) the 05:00Z wave still died — Postgres logs showed the instance **IO-bankrupt** (a 7-row job_runs seq scan took 104s; pg_cron "job startup timeout" across ALL jobs; Supavisor session pool jammed by dead isolates' connections) → **both panel crons PAUSED via cron.alter_job** (load-shed while the disk IO budget refills); (4) **7 wedged job_runs rows marked failed**. Re-enable the panels once capture has been healthy ~30 min (v4 + 0076 make the tick cheap thereafter). Recovery being watched; the 21-day baseline re-run resumes when the DB breathes.
> - **⚠⚠⚠ 03:57Z: capture FULLY STALLED — zero rows since 02:26Z.** Every tick still walks Gamma/CLOB fine (captured:~98) but the terminal batch insert times out EVERY run now — even between the panel's death-windows; one run couldn't even write `complete_job_run` ("upstream request timeout"). Small writes (whale-watch, 1-min cadence) still land → consistent with **disk-IO burst-credit exhaustion** from ~4.5h of the panel's ~GB-scale spilling sorts (not just transient contention). Expect the instance to stay sluggish until the death-cycle stops + credits refill. **Morning sequence unchanged and now the FIRST thing to do: apply 0076 → redeploy maker-exit-panel → watch capture inserts resume (and if the instance stays IO-starved, give it ~30–60 min before judging).** Overnight guardrail (no prod changes) held: nothing was touched; all findings queued here.
> - **⚠⚠ 02:55Z: capture degradation deepened — ~1 inserting tick/hour** (99 rows/h vs healthy ~1,300+). Mechanism fully pinned: every tick walks Gamma/CLOB fine (captured:99) but runs 118–360s through the seed RPCs during the panel's death-windows (~28 min/hour saturated), then the FINAL insert RPC times out → `inserted:0` on ~4 of 5 ticks. **Ticks at 01:50/01:52Z observed `flatOpen:1` (the 07-04 open!) and lost the rows.** The effective capture cadence is collapsed until actions #1+#2 run.
> - **⚠ URGENCY ESCALATION (01:50Z): the broken panel is now degrading CAPTURE.** From ~01:19Z (the daily Gamma batch grew the universe 62→97 incl. 16 fresh-open → more per-tick seed work) the panel's 15-min death-windows (~400s of heavy DB each ≈ 26 min/hour saturated) started starving opening-capture: ticks stretched 15s→130–370s, seed RPCs threw statement timeouts (`get_build_inputs`/`upsert_bucket`/`bot_seed_quality`), and two 'ok' ticks landed **inserted:0** (no capture rows 01:24→01:50Z). CAPTURE_DEADMAN + BOT_DEADMAN fired CORRECTLY — note the 00:20/00:30Z alarms have `sent=true` (the 0066 allowlist passes deadman kinds through the Slack pause → **the operator's Slack WAS paged overnight**; later repeats deduped). Capture recovers between death-windows (effective cadence ~6–10 min, tolerable for the 20-min replay; fresh-open sampling thinned, not zeroed). Do actions #1+#2 FIRST thing — they remove the saturation source. If tick health doesn't normalize within ~30 min after, also consider a one-tick pause/unpause of the `maker-exit-panel` cron to clear any wedged claim rows.
