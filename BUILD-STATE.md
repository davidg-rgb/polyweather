...
> **2026-06-27 (later): 5-ROUND multi-agent code review of the Phase 0/1 build → ~50 findings fixed; suite 1605 green, typecheck clean. NOT committed; NOT yet re-deployed to prod.**
...
> - **⚠ DEPLOY-PROPAGATION (the fixes are source-only so far):** migration 0066 + the `opening-capture` fn are ALREADY LIVE on prod from the morning build, and these fixes change both. For them to take effect on prod: (1) re-apply the changed 0066 DB objects (the wrapped RPCs, lead-aware `bot_seed_quality` [+drop the 2-arg], `bot_deadman_check`, the Slack-allowlist do-block) via `execute_sql`; (2) **`update config set value='6' where key='bot.gate.minCities';`** (the mirror's `on conflict do nothing` won't auto-update an applied 0066 — but this only bites at the far-downstream Phase-5 gate); (3) redeploy the `opening-capture` edge fn (seed.ts now calls the 3-arg `bot_seed_quality` — re-apply the SQL FIRST). All keyless. Operator-gated.
